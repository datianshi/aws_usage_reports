...
do
  ...
  aws ec2 describe-instances --output=json --region=${region} | jq -r '[.[][].Instances[] | {InstanceId, LaunchTime: .LaunchTime, Date: .LaunchTime|sub(".[0-9]+Z$"; "Z") | fromdate, AZ: .Placement.AvailabilityZone, KeyName, State: .State.Name}]' | jq 'sort_by(.Date)'
